                    ...
                    if os.path.isfile(path):#是文件
                        ...
                        if "." in path:
                            ...
                            if checkp=="jpe" or checkp=="jpeg" or checkp=="jpg" or checkp=="gif" or checkp=="png" or checkp=="tif" or checkp=="tiff" :
                                htm=htm.replace("[file]",'<img src="%s" alt="%s" width="100" height="100"/>[file]'%(real_url,file_list[i]))
                                htm=htm.replace("[file]",'<a href="%s">%s</a><br>[file]'%(real_url,file_list[i]))
                            else:#非图片
                                htm=htm.replace("[file]",'<a href="%s">%s</a><br>[file]'%(real_url,file_list[i]))
                        else:#无扩展名
                            htm=htm.replace("[file]",'<a href="%s">%s</a><br>[file]'%(real_url,file_list[i]))
                    else:#是文件夹
                        htm=htm.replace("[path]",'<a href="%s">%s</a><br>[path]'%(real_url,file_list[i]))
                htm=htm.replace("[link]",'<a href="http://192.168.1.106:8080/upload/%s">在此上传</a><br>[link]'%name)
                htm=htm.replace("[link]",'<a href="http://192.168.1.106:8080/newfile/%s">新建文件夹</a><br>'%name)
